...
- Both endpoints return 429 + `Retry-After` header when exceeded. IP extracted via `getConnInfo` from `@hono/node-server/conninfo`; `getClientIp(c)` trusts `X-Forwarded-For` only when the TCP connection comes from a private/internal IP (Docker network) — external clients cannot spoof it. The Next.js proxy forwards the real client IP on all requests.
