...
- **Auth**: `web/middleware.ts` protects all routes except `/login`, `/change-password`, and `/api/auth/*`. On login, two cookies are set: `home-inventory-jwt` (httpOnly, 15min) and `home-inventory-refresh` (httpOnly, 30d). The middleware verifies the JWT via `jose`; if expired, silently refreshes using the refresh cookie (also checks `forcePasswordChange` on the refreshed token) before redirecting to `/login`. `forcePasswordChange=true` in the JWT payload redirects page requests to `/change-password`. If `JWT_SECRET` is not set, middleware allows all (dev mode). Routes: `web/app/api/auth/login/route.ts` (proxies to backend, sets cookies), `web/app/api/auth/logout/route.ts` (revokes refresh token, clears cookies), `web/app/api/auth/change-password/route.ts` (proxies to backend, sets new JWT + refresh cookies from response), `web/app/change-password/page.tsx` (change-password form — calls `/api/auth/change-password` not the generic proxy so cookies are updated). Proxy at `web/app/api/proxy/[...path]/route.ts` reads `home-inventory-jwt` cookie and forwards as `Authorization: Bearer <jwt>`; falls back to `X-API-Token` if no JWT (dev). The proxy also honours an `x-refreshed-jwt` request header (set ONLY by the middleware's silent-refresh path, since the in-request cookie is still the expired one) — the middleware **strips any client-supplied `x-refreshed-jwt` on every pass-through** (`passThrough()`), so a client cannot inject one to override the httpOnly-cookie Bearer token.
...
web/
  app/
    ...
    valuation/             Insurance valuation report — totals by catalogue + room (client-side
                           ...
                           cells starting with = + - @ are prefixed with ' to block
...
- **`item_attachment`** (server PostgreSQL ONLY — not in mobile SQLite, not synced) — unified store for ALL item files: photos and documents/receipts. Columns: `id`, `item_id` (FK, cascade), `kind` (`'photo' | 'document'`), `original_filename`, `mime_type`, `size`, `is_primary`, `created_at`. Files live at `<IMAGE_PATH>/attachments/<attachmentId>.<ext>` (ext from a mime whitelist, never from the user filename). The item's "primary photo" (thumbnail) is the one flagged `is_primary`, falling back to the OLDEST photo attachment. `POST /api/attachments/:id/primary` sets the flag (clears siblings, bumps item `last_modified`). The list endpoint orders primary-first — clients rely on `photos[0]` being the thumbnail. Image URLs in both clients append `?v=<lastModified>` so a changed primary busts the 1-hour HTTP/image cache. Clients fetch attachment lists over the API on demand (online-only, like photos always were).
...
**Push**: selects all local records where `synced = false`, including `sync_tombstone` rows. Before pushing items, orphaned items whose `catalogue_id` no longer exists locally are **tombstoned then deleted** (a bare delete would let them resurrect from the server on the next pull; the tombstone propagates the delete). Sends unsynced catalogues, items, and tombstones in a single POST to `/api/sync/push`. Records carry their own **edit-time `last_modified`** — the push does NOT restamp it with the sync time (doing so would make "last to sync" win instead of "last to edit"). On success, marks pushed rows `synced = true` **only when their `last_modified` is unchanged** since selection, so an edit made mid-push is left unsynced to push next time rather than silently swallowed.

**Pull**: GET `/api/sync/pull?since={lastSyncAt}`. Server returns catalogues, items, and tombstones modified/deleted since that timestamp. Tombstones are applied first — matching local items/catalogues are deleted before upserts run. Each received record is upserted locally — updated only if the server's `last_modified` >= the local `last_modified` (last-write-wins). Pull does **not** skip a just-pushed row: server-side LWW may have rejected the push and kept a newer version, which must be pulled back (re-applying an accepted own-push is a harmless no-op via the timestamp comparison). Updates `last_sync_at` in the `settings` table.
...
**Server push endpoint** upserts with a **last-write-wins guard**: `onConflictDoUpdate` uses `setWhere: <stored>.last_modified <= excluded.last_modified`, so a stale device syncing late cannot overwrite a newer edit that already reached the server (applies to catalogues and items). It also handles two constraint violations gracefully rather than returning 500:
...
- A catalogue tombstone nulls affected items' `catalogue_id` server-side (note: this path does not yet bump their `last_modified`, so that specific change may not pull to other devices until the item is next modified — known minor gap).
...
- `JWT_SECRET` env var secures all endpoints except `/api/health`, `/api/discover`, `/api/auth/login`, `/api/auth/refresh`. If not set, auth is skipped (dev mode). `verifyJwt` pins `algorithms: ['HS256']`.
- **`forcePasswordChange` is enforced server-side**: the auth middleware rejects any non-`/api/auth/*` request whose JWT carries `forcePasswordChange=true` with `403` — a temp-credential holder cannot reach the data API by calling it directly, only the change-password/refresh/logout endpoints.
- **Request body-size limits** (`hono/body-limit`): uploads and sync-push are capped at 50 MB, restore at `RESTORE_MAX_MB` (env, default 500 MB) — returns `413` on exceed. Bounds memory use since each buffers the whole body.
- **ID validation for filesystem paths** (`isUuid`): the legacy `/api/items/:id/image` routes, `/api/items/:id/attachments`, and restore all reject non-UUID ids before building any file path — closes path-traversal via URL param or crafted backup JSON. DB-sourced ids are already safe (uuid columns).
...
- Login uses **exponential backoff** rate limiting: 5 failures → blocked 2 min; another 5 → 4 min; another 5 → 8 min, etc. (capped at 1 hour). Successful login resets the counter. Implemented via `checkLoginBlocked` / `recordLoginFailure` / `recordLoginSuccess` with an in-memory `loginState` Map (entries expire after 24h of inactivity).
- `POST /api/auth/refresh` uses a simple fixed-window limiter: 20 attempts per IP per 15 minutes (`createRateLimiter`).
- Both endpoints return 429 + `Retry-After` header when exceeded. IP extracted via `getConnInfo` from `@hono/node-server/conninfo`; `getClientIp(c)` only trusts forwarding headers when the TCP peer is a private/internal IP (the reverse proxy / Docker network), and then prefers **`X-Real-IP`** (set by the Synology reverse proxy, not client-spoofable) else the **rightmost** `X-Forwarded-For` hop — never the client-controllable leftmost, so an attacker cannot spoof a fresh IP per request to bypass the login limiter. The Next.js proxy derives the client IP the same way (`x-real-ip` first) and forwards it as a single clean `X-Forwarded-For` + `X-Real-IP`. **Depends on the Synology reverse proxy setting `X-Real-IP`** (its default).
- `/api/auth/logout` is in the public paths list (no token required — you may be logging out because your token expired).
- `POST /api/auth/change-password` updates the password, clears all existing refresh tokens, issues a new JWT (with `forcePasswordChange=false`) and a new refresh token, returns `{ ok, token, refreshToken }`. The web route at `web/app/api/auth/change-password/route.ts` sets the new cookies so the middleware sees the updated JWT immediately.
...
- **Attachment endpoints**: `GET /api/items/:id/attachments` (list, primary-first), `POST /api/items/:id/attachments` (multipart `file` + optional `kind`; defaults to `photo` for `image/*` mimes), `GET /api/attachments/:id/file` (serve with Content-Disposition), `DELETE /api/attachments/:id`, `POST /api/attachments/:id/primary` (make a photo the thumbnail). All token-protected. **Uploads are restricted to the `EXT_BY_MIME` whitelist** (jpg/png/webp/heic/pdf) — a non-whitelisted mime (e.g. `text/html`) is rejected `400`, so it can never be stored and served inline (blocks stored XSS). The file serve sends `X-Content-Type-Options: nosniff` and uses `Content-Disposition: attachment` for documents (`inline` for photos). `IMAGE_PATH` env var defaults to `./images`; in prod it is `/images` (mapped to Docker volume); attachment files under `<IMAGE_PATH>/attachments/`. Mobile helpers in `sync.ts`: `listAttachments`, `uploadAttachment`, `deleteAttachment`, `getAttachmentUrl` (same `{ url, headers }` shape as `getImageUrl`). Web: `api.attachments` in `web/lib/api.ts`. Mobile opens documents by downloading to cache via `expo-file-system/legacy` `downloadAsync` (Bearer headers) then `expo-sharing` `shareAsync` — a Bearer-protected URL can't be opened directly in a browser/QuickLook.
