...
- `POST /api/auth/login` rejects passwords over 256 characters before hashing (prevents DoS via slow scrypt on huge payloads) and trims the username before the DB lookup. The web login form also trims username before posting. Rate-limited to 5 attempts per IP per 15 minutes.
- `POST /api/auth/refresh` is rate-limited to 20 attempts per IP per 15 minutes (higher than login since a valid token is required, but still bounded).
- Both auth endpoints use a shared `createRateLimiter(max, windowMs)` factory — each returns an independent in-memory counter with its own cleanup interval. Returns 429 + `Retry-After` header when exceeded. IP extracted via `getConnInfo` from `@hono/node-server/conninfo` (TCP connection address — not spoofable via headers); wrapped in a `getClientIp(c)` helper.
